# Lesson 19: Framebuffers and Image Views
In the last lesson we've created the swapchain that contains the images we need to render to, but we're still missing the framebuffers that will establish the connection between our graphics pipeline and those images. This is what we're going to take care of today, and along the way we'll also introduce image views, which are the abstraction Vulkan uses to refer to those images.
...
- we already came across the concept of 'image array layers' in the previous lesson. The `layers_` parameter here is equivalent to `imageArrayLayers` in `SwapchainCreateInfoKHR`
...
```
The image views need to be persistent, so we cannot create them directly from within `createFramebuffers`. That's why we need to pass them in and create our framebuffers in `main` like so:
```cpp
