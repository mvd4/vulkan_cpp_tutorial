...
```
This time we need to set the `eResetCommandBuffer` flag because we're going to re-record our command buffers each frame[^1]. Without that flag the only way to reset a buffer would be to reset the whole pool; with it, calling `begin()` on an already-recorded buffer implicitly resets just that one.

From the pool we can now create the actual command buffers. We're going to create as many as the number of frames we asked for when creating the swapchain[^2] (Note that I renamed the variable to make its meaning even more explicit):
```cpp
...
```
The `SubpassContents` parameter defines where the subsequent commands for the subpass(es) come from. They can be recorded directly into the current command buffer or executed from a secondary command buffer. We're going to record them directly into the command buffer.
