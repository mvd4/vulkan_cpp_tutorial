...
So we have a physical device here that offers quite a few features like e.g. anisotropic filtering, texture compression, tessellation and geometry shaders, and so on. The application on the left configures its logical device to only use anisotropic filtering and sample shading. All the other features of the physical device are being left switched off. The application on the right on the other hand does not turn on any of the graphics features for its logical device, potentially because it only wants to use the compute capabilities.
...
One final note before we actually start implementing: while it is possible to create multiple logical devices from the same physical device, there is no good reason to do so. You won't get any better parallelization than by using one logical device with multiple queues, it is more work and you lose some options for synchronization and data transfer. Multiple logical devices only make sense when you want to work with multiple physical devices at the same time. But that's something we won't go into in this tutorial.
...
{
    ...
    vk::UniqueDevice createDeviceUnique( const vk::DeviceCreateInfo&, ... );
    ...
};
```
...
struct DeviceCreateInfo
{
    ...
    DeviceCreateInfo& setPEnabledLayerNames( const container_t< const char* const >& layerNames_ ); // deprecated, kept for backwards-compatibility
    DeviceCreateInfo& setPEnabledExtensionNames( const container_t< const char* const >& extensionNames_ );
    DeviceCreateInfo& setPEnabledFeatures( const PhysicalDeviceFeatures* pEnabledFeatures_ );
    DeviceCreateInfo& setQueueCreateInfos( const container_t< DeviceQueueCreateInfo >& queueCreateInfos_ );
    ...
};
```
The first property we can set are again some flags. Since Vulkan 1.1, this can be used to enable protected memory functionality, but that's an advanced topic we won't cover in this tutorial. For now we can ignore the method.
...
struct DeviceQueueCreateInfo
{
    ...
    DeviceQueueCreateInfo& setQueuePriorities( const container_t< const float >& queuePriorities_ );
    ...
};
```
The `flags` parameter can be used to create protected queues (since Vulkan 1.1), which is an advanced topic we won't cover here.
...
```
`queueCount` gives us the maximum number of queues that you can create from this family. `queueFlags` is a bitfield (see [lesson 1](lesson%201.md)) that tells us which operations this queue supports. The possible flag bits are:
...
Compile and run this and you will get output along those lines[^3]:
```text
Available queue families:
    ...
    Queue Family 3:
        ...
        supports sparse binding operations: no
```
In this case it seems there are multiple queue families with identical capabilities. This might be due to some internal differences between the queues which cannot be expressed via the Vulkan interface. Another reason might be that queues from the same family can share resources with minimal overhead. So an implementation might expose multiple queue families with the same capabilities when the underlying hardware cannot efficiently parallelize work within a single queue family.

While in many cases you might get away with simply selecting the first queue family, you should not rely on that being the one that supports the functionality you need. You might also find that there actually is no single queue family that has everything you need, in this case you'll have to use different queues for different tasks.

We'll again keep it relatively simple in this tutorial and just assume that the physical device has a queue family that fulfills all our needs and select that[^4]:
```cpp
...
```
And with this helper function in place we can finally fill our `QueueCreateInfo` structure. We just have to remember that we're obliged to set the priorities, even if we have just one single queue[^5].
```cpp
...
[^3]: On macOS, you may see a warning message like "flock failed to lock list file" mentioning `com.apple.metal`. This is a harmless warning from Apple's Metal framework (which MoltenVK uses to implement Vulkan on macOS) related to its shader cache. It does not affect functionality and can be safely ignored. If it bothers you, try clearing Metal's cache by running `rm -rf ~/Library/Caches/com.apple.metal` in the terminal.
[^4]: In the original Vulkan tutorial, the available queues are actually already queried when selecting the physical device. This is definitely the more robust way to do it, and I recommend considering this approach for any production code. This is a tutorial, so I try to keep things simple by assuming the queues we need are available on the selected device (I actually did not yet encounter a system that supported Vulkan and didn't have a graphics and a compute queue family)
[^5]: In this case the function expects an `ArrayProxyNoTemporaries` (see [lesson 1](lesson%201.md)), therefore we cannot pass an rvalue to the function.
