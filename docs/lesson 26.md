...
That is of course not enough. A depth test needs somewhere to store and compare against the depth values. Storing a depth value per fragment isn't really different than creating a grayscale image, so Vulkan uses an image attachment to store the depth values. We did not create one so far though, let's change that.

The depth format will appear in several places (the render pass, the depth image, and the depth image view), so let's introduce a single named constant for it in `presentation.hpp`:
...
We only need one float value per fragment, hence the `eD32Sfloat`[^2] - the `S` here stands for *signed*. With that in place we can extend the render pass:
```cpp
...
Of course. Our pipeline now expects the framebuffer to come with a depth attachment, which of course it doesn't have yet. Let's fix that by pretending we already have one depth image view per swapchain image that we could pass to the framebuffer creation function:
...
```
We give every framebuffer its own depth image and image view, just like each one already has its own color image view. Since we already have synchronized access to the color image views in our loop, we don't need to worry about synchronization issues here.

So far, so good, unfortunately we don't have any depth image views yet. Where do we get those from? The `imageViews` for the rendering output are members of our `Swapchain` class, so it seems logical that it also owns the depth images. However, while the former are created for us by the `SwapchainKHR` class we will have to do it ourselves for the depth images. The logical device gives us what we need:
...
We're not done here though. Remember how it wasn't enough to create a `vk::Buffer` but instead we needed to explicitly allocate the memory for it? We need to do exactly the same here. Let's create a struct similar to `GPUBuffer` but for images:
```cpp
struct GPUImage
{
...
... and expand our `createDepthImage` function to also allocate the memory:
...
With that function we can create the images, but `createFramebuffers` needs `ImageView`s. Well, that shouldn't be too hard, we already have the `createImageView` helper. The problem is that this function currently has the `ImageAspectFlags` hardcoded to `eColor`. We need to set a flag of `eDepth`, so we need to expose that parameter:
...
As said above, the swapchain should own the depth images and their image views and pass them on to the framebuffer creation. Since we want one depth image per swapchain image, we wrap `createDepthImage` and `createImageView` in two small helpers that build a `std::vector` each - one depth image (and view) for every entry in `m_imageViews`:
...
auto createDepthImageViews(
    const vk::Device& logicalDevice,
    const std::vector< GPUImage >& depthImages
...
Now we need to adjust the `Swapchain` constructor a bit to actually create them. Note that since `m_framebuffers` needs the depth image views, we have to make sure that the depth image and view members are declared (and therefore initialized) *before* `m_framebuffers` in the class definition:
```cpp
Swapchain::Swapchain(
    const vk::PhysicalDevice& physicalDevice,
    const vk::Device& logicalDevice,
...
Once again the fact that Vulkan really only does what we ask it to do bites us: we have created a depth attachment for our render pass, but we didn't specify a clear value for that attachment. That means that the depth buffer contains arbitrary garbage and so the depth testing cannot work properly. The solution is luckily trivial: we already specify a clear value for the color attachment, so we just need to extend that code a bit:
...
![Screenshot showing a perspective rendering of the cube without errors](images/Screenshot_5_Cube_3.png "Fig. 1: We finally see a correctly rendered cube")
