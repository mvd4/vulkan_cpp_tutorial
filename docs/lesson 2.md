# Lesson 2: A brief overview of the Vulkan C++ wrapper
...
The structs, classes and functions provided by this header are convenience wrappers that add exception handling, RAII support, and type safety on top of the C API. So in general it should be pretty easy to find your way around the C++ interface. However, I think it's useful to point out a few concepts and patterns that will make it easier to follow this tutorial.

## Namespace
Everything in the Vulkan C++ wrapper is by default located in the namespace `vk`. This namespace replaces the `Vk...` prefix of the functions and classes in the Vulkan C-API. So, e.g., the C++ equivalent to Vulkan's `vkCreateInstance` is `vk::createInstance`.
...
The Vulkan C-API uses result-codes for error handling. Most functions return `VK_SUCCESS` if they complete without errors and an error code otherwise. By default the C++ wrapper converts those error codes into exceptions. Therefore, C++ wrapper functions do not need to use out-parameters and instead can return their result directly. E.g. this Vulkan C function
```c
VkResult vkCreateInstance( const VkInstanceCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkInstance* pInstance );
```
becomes the following in the C++ wrapper:
```cpp
vk::Instance createInstance( const vk::InstanceCreateInfo& createInfo, vk::Optional< const vk::AllocationCallbacks > allocator /*, dispatch */ );   // will throw in case of error
```
...
Some Vulkan functions may return a result that is not really an error, but neither has the function completed fully successfully. A typical example is acquiring a swapchain image: the call may succeed, yet return `vk::Result::eSuboptimalKHR` to signal that the swapchain no longer matches the surface perfectly. The C++ wrapper would be wrong in throwing an exception for this, but it would equally be wrong to silently ignore the result. For those cases the C++ wrapper uses `vk::Expected<T>`, which is modelled after the C++23 `std::expected` class template:
```cpp
template <typename T>
class Expected
{
public:
    operator bool() const;    // true if the call produced a value
    bool has_value() const;   // same as operator bool()

    T &        value();       // access the wrapped value (asserts/throws if no value)
    T &        operator*();   // same as value()

    vk::Result error() const; // the result code when no value is held
};
```
`vk::Expected<T>` holds *either* a value of type `T` *or* a `vk::Result` code — never both at the same time. You check which one it holds with `has_value()` (or `operator bool()`), retrieve the value with `value()` (or `operator*`), and retrieve the result code with `error()`. This will be familiar to anyone who has used structured bindings and the `std::optional`/`std::expected` idioms in modern C++.

For clarity, I'll simplify the return type to the `T` type when showing `vk` function signatures in most cases.

## Vulkan constants and flags
There is a vast number of constants and flags defined by Vulkan C-API. The C++ wrapper converts all of those to scoped enumerations, so you'd e.g. use `vk::BlendFactor::eOneMinusSrcColor` instead of the original Vulkan constant `VK_BLEND_FACTOR_ONE_MINUS_SRC_COLOR`. That doesn't save you much typing, but it adds type-safety and a bit of readability (at least in my opinion).
...
However, while you could define those operators as freestanding functions on the scoped enums, combining two flag bits with `|` would still yield an enum value that doesn't correspond to any named enumerator. The creators of the Vulkan wrapper therefore introduced the `...Flags` classes which wrap the corresponding `...FlagBits` and provide a proper bitmask type that can represent arbitrary combinations of flags, along with the full set of bitwise operators.
This probably sounds a bit complicated, but for the users of the SDK it adds a lot of convenience as they can use the `...FlagBits` just as they'd use plain old bitmasks and still benefit from the advantages of C++ scoped enums.
...
## RAII
One of the most important patterns to improve the clarity and correctness of C++ code is [RAII](https://en.cppreference.com/w/cpp/language/raii) (Resource Acquisition is Initialization). A good C++ library should therefore support this pattern.
...
The creators of the Vulkan C++ wrapper chose another way. The classes and structs themselves do not release their resources automatically. To achieve RAII behaviour you have to wrap them in a template class called `vk::UniqueHandle`. This class is very similar to `std::unique_ptr` in both its interface and behaviour:
...
* `UniqueHandle`s can be moved but not copied
* when destroyed, `UniqueHandle`s automatically call the correct Vulkan function to free the underlying resources
...
The [named parameter idiom](https://isocpp.org/wiki/faq/ctors#named-parameter-idiom) is an emulation of a language feature that is available in other languages (e.g. Python, Dart, Ruby ) [^2]. It also has quite a few similarities with the classic builder pattern, only that it doesn't require any additional classes. The basic idea is that instead of initializing all fields of a class/struct directly via constructor arguments, each field is initialized explicitly via a setter function. The trick is that the setters return a reference to the object itself (just like the assignment operators do) and thus can be chained. So you can initialize every struct in the Vulkan C++ wrapper using this pattern:
```cpp
const auto appInfo = vk::ApplicationInfo{}
    ...
    .setEngineVersion( 1u )
    .setApiVersion( vk::makeApiVersion( 0, 1, 1, 0 ) );
```
...
To my knowledge, the only difference between the two classes is that `vk::ArrayProxyNoTemporaries` prevents binding to temporary objects. It achieves this through SFINAE constraints that require lvalue references on its constructors, so you can't accidentally pass in temporaries that would dangle.
...
The core Vulkan API is very stable, and so the Vulkan loader (a dynamic library you link against at build time) exposes all core functions as regular symbols that are resolved at link time - this is called *static dispatch*. The Vulkan C++ wrapper can simply call those functions directly. The same is true for some of the most common extensions - they are available through the loader. However, if you want to use functions from extensions that are not included, you need to find a way to make them accessible. This is done by loading their function pointers at runtime via `vkGetInstanceProcAddr` or `vkGetDeviceProcAddr` - called *dynamic dispatch*.

The C++ wrapper provides maximum flexibility here by giving every function that calls into the Vulkan API the `Dispatch` argument which allows you to customize the loading. It defaults to using static dispatch as described above, and we'll just leave it at that whenever we can. This is why I'll also usually omit it when I show function signatures.
...
[^2]: C++20 introduced designated initializers which allow you to initialize aggregate members by name (e.g. `.fieldName = value`). While not exactly the same as named parameters (the initializers must appear in declaration order), they serve a similar purpose. The Vulkan C++ interface can be configured to work with those as well. We'll not do that in this tutorial and stick with the setter-based approach for consistency.